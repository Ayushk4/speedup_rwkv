...
#moEXPRESS_PILE_MODEL_TYPE = 'RWKV-4-Pile-1B5'

########################################################################################################

datafile = "../data/pile_00/pile_00_text_document" # your data
datafile_encoding = 'binidx' # 'utf-8' / 'utf-16le' / 'numpy' (for fine-tuning pile models) / 'binidx' (the Megatron-LM 'binidx' format)
...
# if you see "CUDA out of memory", reduce batch_size.
...
# 2) Check epoch_save_frequency and make sure the partially-trained model is saved.
...
# 
# For L12-D768, set lr_init = 6e-4. For L24-D1024, lr_init = 4e-4. For L24-D2048, lr_init = 3e-4.
...
n_epoch = 500
epoch_length_fixed = (10000 // batch_size) * batch_size # you may increase if you have multiGPUs
...
if __name__ == '__main__':
    ...
    print('\nmodel', model_type, os.environ['RWKV_FLOAT_MODE'], 'epoch', n_epoch, 'batchsz',
          batch_size, 'betas', betas, 'eps', eps, 'ctx', ctx_len, 'layer', n_layer, 'embd',
          n_embd, '\n')

    tconf = TrainerConfig(
        model_type=model_type, max_epochs=n_epoch, batch_size=batch_size, learning_rate=lr_init,
        lr_decay=True, lr_final=lr_final, betas=betas, eps=eps, warmup_tokens=warmup_tokens,
        final_tokens=n_epoch*len(train_dataset)*ctx_len, num_workers=num_workers,
    ...
    if os.environ['RWKV_DEEPSPEED'] == '0':
        ...
        if os.environ['RWKV_FLOAT_MODE'] == 'fp16':
            ...
            trainer = Trainer(strategy=DeepSpeedStrategy(config=DEEPSPEED_CFG), devices=NUM_GPUS,
